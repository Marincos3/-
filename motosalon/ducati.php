...
<div id="moto" class="featur text-centerd ">
  ...
  <img src="images/m-d1.jpg" class="imgy" alt="...">
  <div class="card-body">
    <h5 class="card-title"><?php $name = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM name WHERE id_name = 3"));
    ...
    <a href="duc1.php" class="btn btn-primary" target="_self"><?php $tovar = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM tovar WHERE id_tovar = 3")); $_SESSION['tovar'] = [ "price" => $tovar['price'] ]; ?><?= $_SESSION['tovar']['price'] ?>$</a>
  </div>
</div>
          </div>
          <div class="cole feat">
            <div class="card" style="width: 18rem;">
  <img src="images/m-dd1.jpg" class="imgy" alt="...">
  <div class="card-body">
    <h5 class="card-title"><?php $name = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM name WHERE id_name = 8"));
      $_SESSION['name'] = [ "brand" => $name['brand'], "model" => $name['model'] ]; ?><?= $_SESSION['name']['brand'] .' '. $_SESSION['name']['model']?></h5>
    <p class="card-text">Тут може бути додаткова інформація.</p>
    <a href="duc2.php" class="btn btn-primary" target="_self"><?php $tovar = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM tovar WHERE id_tovar = 8")); $_SESSION['tovar'] = [ "price" => $tovar['price'] ]; ?><?= $_SESSION['tovar']['price'] ?>$</a>
  </div>
          </div>
